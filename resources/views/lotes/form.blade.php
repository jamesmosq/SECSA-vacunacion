@extends('layouts.app')
@section('title', isset($lote) ? 'Editar Lote' : 'Nuevo Lote')
@section('content')
<div class="d-flex justify-content-between align-items-center mb-3">
    <h4>{{ isset($lote) ? 'Editar Lote: '.$lote->lote : 'Nuevo Lote' }}</h4>
    <a href="{{ route('lotes.index') }}" class="btn btn-secondary">Volver</a>
</div>

<div class="card">
    <div class="card-body">
        <form method="POST" action="{{ isset($lote) ? route('lotes.update', $lote->lote) : route('lotes.store') }}">
            @csrf
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <select name="insumo" class="form-select @error('insumo') is-invalid @enderror" required>
                        <option value=""></option>
                        @foreach($insumos as $ins)
                            <option value="{{ $ins }}" {{ old('insumo', $lote->insumo ?? '') == $ins ? 'selected' : '' }}>{{ $ins }}</option>
                        @endforeach
                    </select>
                    @error('insumo') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>

                <div class="col-md-4">
                    <label class="form-label">Presentación *</label>
                    <select name="presentacion" class="form-select @error('presentacion') is-invalid @enderror" required>
                        <option value=""></option>
                ...
                <div class="col-md-2">
                    ...
                    <input type="number" name="saldo" class="form-control @error('saldo') is-invalid @enderror"
                           value="{{ old('saldo', $lote->saldo ?? 0) }}" min="0" required>
